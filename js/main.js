...
function serviceInfo() {
  $('.js-service-toggle').on('click', function () {
    if ($(this).hasClass('active')) {
      $(this).removeClass('active').next('.js-service-item__cnt').slideUp('slow', 'linear');
    } else {
      $('.js-service-toggle').removeClass('active').next('.js-service-item__cnt').slideUp('slow', 'linear');
      $(this).addClass('active').next('.js-service-item__cnt').slideDown('slow', 'linear');
    }
  });
}
/* Project slider */
...
function projectSlider() {
  ...
  $('.js-project .swiper-button-next').css({
    'right': ($(window).innerWidth() - $('.js-project .swiper-button-next').closest('.section').find('.container').innerWidth()) / 2 + 15
  });
  $('.js-project .swiper-button-prev').css({
    'right': ($(window).innerWidth() - $('.js-project .swiper-button-prev').closest('.section').find('.container').innerWidth()) / 2 + $('.swiper-button-next').innerWidth() + 45
  });
}
/* Reviews slider */
...
function videoPlay() {
  $(".js-video-btn").on("click", function () {
    ...
    $(this).find(".js-video-content").append("<iframe src='" + $(this).attr('data-link') + "?autoplay=1' frameborder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe>");
  });
}
/* Hero slider */
...
function longImg() {
  $('.js-long-img').each(function () {
    $(this).css({
      'width': (window.innerWidth - $('.container').innerWidth()) / 2 + $(this).parent('div').innerWidth() + parseInt($('.container').css('padding-right'))
    });

    if ($(this).hasClass('js-img-left')) {
      $(this).css({
        'margin-left': -(window.innerWidth - $('.container').innerWidth()) / 2 - parseInt($('.container').css('padding-left'))
      });
    }
  });
}
...
function timer() {
  ...
  function timeBetweenDates(toDate) {
    ...
    if (difference <= 0) {
      ...
      minutes %= 60;
      seconds %= 60;
      if (days < 10) $("#days").text('0' + days);else $("#days").text(days);
      if (hours < 10) $("#hours").text('0' + hours);else $("#hours").text(hours);
      if (minutes < 10) $("#minutes").text('0' + minutes);else $("#minutes").text(minutes);
      if (seconds < 10) $("#seconds").text('0' + seconds);else $("#seconds").text(seconds);
    }
  }
}
/* Masonry */
...
function scrollDownPos() {
  $('.js-scroll-down').css({
    'left': (window.innerWidth - $('.js-scroll-down').closest('.hero').find('.container').innerWidth()) / 2 + 15
  });
}
...
function map() {
  if ($(".js-map").length > 0) {
    var mapScript = $('<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDHS8cBjXUwNpyPHDiwTkXniVmeReTGr38&language=en-EN&callback=initMap" async defer></script>');
...
function newsHover() {
  $('.js-news-link').on('mouseenter', function () {
    ...
    $('.js-news-img').eq($(this).closest('.js-news-item').index()).addClass('active');
  });
} // function mixItUp() {
//     if($('.js-list').length) {
